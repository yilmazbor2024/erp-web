...
const CashPaymentListPage: React.FC = () => {
  ...
  const fetchPayments = async (page = 1, pageSize = 10) => {
    ...
    try {
      ...
      const response = await cashVoucherApi.getCashPaymentVouchers(params);
      ...
      if (response.success) {
        ...
        message.error(response.error || 'Kasa tediye fişleri yüklenirken bir hata oluştu');
      }
    } catch (error) {
      console.error('Kasa tediye fişleri yükleme hatası:', error);
      message.error('Kasa tediye fişleri yüklenirken bir hata oluştu');
    } finally {
      setLoading(false);
    }
  };
      ...
      render: (_: any, record: any) => {
        ...
        return (
          <div style={{ display: 'flex', flexDirection: 'column', borderBottom: '2px solid #fa8c16', paddingBottom: '10px', marginBottom: '10px' }}>
            ...
            {/* Açıklama - 3 satır ile sınırlandırılmış ve renkli */}
            <div 
              style={{ 
                marginTop: '8px', 
                padding: '6px', 
            ...
            >
              ...
              <div style={{ marginLeft: '8px', alignSelf: 'center' }}>
                <Button 
                  type="primary" 
                  size="small"
                  onClick={() => navigate(`/cash/payment/detail/${id}`)}
                >
                  Detay
                </Button>
              </div>
    ...
    <div>
      <div style={{ marginBottom: 16, padding: '16px 24px', backgroundColor: '#fff' }}>
        <Row justify="space-between" align="middle">
          <Col>
            <Title level={4} style={{ margin: 0 }}>Kasa Tediye Fişleri</Title>
          </Col>
          <Col>
            <Button 
              type="primary" 
