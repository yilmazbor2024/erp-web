...
const CashTransferPage: React.FC = () => {
  const navigate = useNavigate();

  const handleSuccess = (response: any) => {
    message.success('Kasalar arası virman işlemi başarıyla kaydedildi');
    ...
    <div>
      <div style={{ marginBottom: 16, padding: '16px 24px', backgroundColor: '#fff' }}>
        <Space>
          ...
          />
          <div>
            <Title level={4} style={{ margin: 0 }}>Kasa Virman Fişi</Title>
            <div style={{ color: 'rgba(0, 0, 0, 0.45)' }}>Yeni kasa virman fişi oluştur</div>
          </div>
        </Space>
      </div>
      <Card>
        <CashVoucherForm 
          voucherType={CashVoucherType.TRANSFER}
